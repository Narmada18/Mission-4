  ...
  try {
    ...
    if (minprice || maxprice)
      filteredCar_data = filteredCar_data.filter(
        (el) => el.oemSpec.listPrice >= Number(minprice) && el.oemSpec.listPrice <= Number(maxprice)
      ); // filter on price

    if (minmileage || maxmileage)
      filteredCar_data = filteredCar_data.filter(
        (el) => el.oemSpec.mileage >= Number(minmileage) && el.oemSpec.mileage <= Number(maxmileage)
      ); // filter on mileage
